...
public class TwoFactorCommand implements CommandExecutor {
    ...
    private FlorestDiscordPro telegramPRO;

    public TwoFactorCommand(FlorestDiscordPro plugin) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (args.length == 0) {
            sender.sendMessage(ChatColor.RED + "Command usage: /2fa <enable/disable/code> <username discord/code>");
            return true;
        }
        String arg = args[0];
        if (arg.equalsIgnoreCase("disable")) {
            ...
            if (args.length == 1) {
                sender.sendMessage(ChatColor.RED + "Usage: /2fa enable <discord username>");
                return true;
            }
            telegramPRO.getMethods().sendDiscordMessageUser(args[1], telegramPRO.getConfig().getString("2fa_connected").replace("{player}", sender.getName()));
